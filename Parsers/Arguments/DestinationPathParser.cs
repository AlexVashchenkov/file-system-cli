...
public class DestinationPathParser<T> : ParserBase<T>
    where T : IWithDestinationPath<T>
{
    public override ArgumentsParsingResult<T> Parse(T builder, IEnumerator<string> enumerator, Context context)
    {
        ...
        if (context.Directory.GetPathType(enumerator.Current))
            builder.WithDestinationPath(new AbsolutePath(enumerator.Current));
        else
            builder.WithDestinationPath(new RelativePath(enumerator.Current));
        ...
        if (Successor is null)
        {
            ...
            return new ArgumentsParsingResult<T>.Failure("Destination path argument parsing error: unable to parse");
        }
