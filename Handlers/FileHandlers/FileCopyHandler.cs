...
{
    private readonly IParser<FileCopyBuilder>? _parser;

    public FileCopyHandler(string name, IParser<FileCopyBuilder>? parser)
    {
        FunctionName = name;
        _parser = parser;
    }
    ...
    public override CommandParsingResult Handle(IEnumerator<string> enumerator, Context context)
    {
        if (enumerator.Current == FunctionName)
        {
            ...
            if (_parser is not null)
            {
                ...
                if (parsingResult is ArgumentsParsingResult<FileCopyBuilder>.Failure failureResult)
